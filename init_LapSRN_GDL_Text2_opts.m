function opts = init_LapSRN_GDL_Text_opts(scale, depth,gdl_lambda, gpu)
% -------------------------------------------------------------------------
%   Description:
...
% -------------------------------------------------------------------------
    ...
    opts.lr                 = 1e-6; %1e-5;     % initial learning rate
    opts.lr_step            = 50; %50      % number of epochs to drop learning rate
    opts.lr_drop            = 0.5;      % learning rate drop ratio
    opts.lr_min             = 1e-7; %1e-6;     % minimum learning rate
    opts.patch_size         = 128;
    ...
    %% dataset options
    opts.train_dataset          = {};
    opts.train_dataset{end+1}   = 'EN100';
    opts.train_dataset{end+1}   = 'FR100';
    opts.train_dataset{end+1}   = 'VN100';
    opts.valid_dataset          = {};    
    opts.valid_dataset{end+1}   = 'Test30';
    ...
    opts.net_name = sprintf('LapSRN_x%d_depth%d_%s_%d', ...
                            opts.scale, opts.depth, opts.loss,opts.gdl_lambda);
